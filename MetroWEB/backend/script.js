  ...
  fetch(`/folder/${folderName}`)
    ...
    .then(images => {
      ...
      images.forEach(img => {
        ...
        runBtn.textContent = "Rodar IA";
        runBtn.onclick = () => {
            fetch(`/inference/${img.id}`)
                .then(res => res.json())
                .then(data => {
                    if (data.status === "ok") {
                    document.querySelectorAll('.resultImg').forEach(e => e.remove());

                    const origImg = document.createElement("img");
                    origImg.src = data.original + "?t=" + new Date().getTime();
                    origImg.classList.add('resultImg');
                    origImg.style.maxWidth = "300px";

                    const overlayImg = document.createElement("img");
                    overlayImg.src = data.overlay + "?t=" + new Date().getTime();
                    overlayImg.classList.add('resultImg');
                    overlayImg.style.maxWidth = "300px";

                    document.body.appendChild(origImg);
                    document.body.appendChild(overlayImg);
                    } else {
                    alert("Erro na inferência: " + data.msg);
                    }
                });
        };
